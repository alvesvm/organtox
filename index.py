                ...
                html.Div(children=[
                    ...
                    dbc.Modal(children=[
                        ...
                        dbc.ModalBody([dcc.Markdown('''
                                       ...
                                       Directly paste the [SMILES](https://en.wikipedia.org/wiki/Simplified_molecular-input_line-entry_system) representation of the desired chemical structure.
                    ...
                    dbc.Modal(children=[
                        ...
                        dbc.ModalBody([dcc.Markdown('''
                                       ...
                                       affected organs in sub-acute and sub-chronic toxicity tests. The web app leverages curated public data on compounds tested orally in 
                                       short-term in vivo assays (1-8 days) that contain body and organ weight, clinical chemistry, and hematology data. 
                                       The data were collected from the [DrugMatrix](https://www.sciencedirect.com/science/article/abs/pii/S0168165605001513) and 
                                       [TG-GATEs](https://academic.oup.com/nar/article/43/D1/D921/2439524) database. 
                                       Curated data comprised 63,814 and 14,971 data points for 
                                       1-8 days and 514 and 126 chemicals, respectively. The integration of clinical chemistry, hematology, organ and body weights 
                                       provided a better indication of appropriate dose selection (i.e., identifying a maximum tolerated dose). 
                                       The tool integrates all the available data implementing the [Multi-Descriptor Read-Across (MuDRA)](https://pubs.acs.org/doi/10.1021/acs.jcim.8b00124) 
...
#url_bar_index = html.Div([dcc.Location(id='url', refresh=False),
#                          html.Div(id='page-content')])

# App layout and validation
app.title = 'OrganTox'
app.layout = layout_index
#app.validation_layout = html.Div([url_bar_index,
#                                  layout_index,
#                                  layout_results])
...
def display_output(smiles, mol):  
    if smiles:
        #chem = smiles
        return 'SMILES inserted.' #'Inserted SMILES: {}'.format(chem)
    elif mol:
        #chem = mol
        return 'Chemical structure drawn.' #'Drawn SMILES: {}'.format(chem)
    else:
...
def load_output(click, smiles, mol):
    if click is None:
        ...
        if smiles:
            ...
            return df.to_json(date_format='iso', orient='split')#, layout_results.layout
        elif mol:
            smol = get_mol(mol)
            df = get_similarity_df(smol)
            return df.to_json(date_format='iso', orient='split')#, layout_results.layout
