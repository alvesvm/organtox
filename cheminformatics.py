...
#print("MAE:", metrics.mean_absolute_error(y_test, pred_test).round(2))
    ...
    def calc_morgan(mol, radius, nBits):
        fp = AllChem.GetMorganFingerprintAsBitVect(mol, radius, nBits, useFeatures=False, useChirality=False)
        fp = pd.Series(np.asarray(fp))
        fp = fp.add_prefix('Bit_')
        return fp
    
    # Calculate AtomPair
    def calc_atom_pair(mol):
    ...
    def calc_avalon(mol, nBits):
        fp = pyAvalonTools.GetAvalonFP(mol, nBits, isQuery=False, bitFlags=pyAvalonTools.avalonSSSBits)
        fp = pd.Series(np.asarray(fp))
        fp = fp.add_prefix('Bit_')
        return fp
